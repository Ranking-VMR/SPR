...
import utils.dense_retrieval_faiss.distributed as distributed
#from utils.dense_retrieval_faiss.runs import Run
from loguru import logger
from utils.dense_retrieval_faiss.utils import timestamp, create_directory
...
class Arguments():
    ...
    def add_index_use_input(self):
        self.add_argument('--index_root', dest='index_root', required=True)
        self.add_argument('--index_name', dest='index_name', required=True)
        self.add_argument('--index_type', help='type of index.', required=True, choices=['ivfpq', 'ivfflat', 'flatl2', 'flatip'])
        self.add_argument('--partitions', dest='partitions', default=None, type=int)
        self.add_argument('--nbits', help='nbits for each quantizing part (only for PQ).', default=8, type=int)
        self.add_argument('--m', help='num of parts for quantizing (only for PQ).', default=16, type=int)
    ...
    def parse(self):
        ...
        if args.nranks > 1:
            logger.info(f"#> Restricting number of threads for FAISS to {args.nthreads} per process",
