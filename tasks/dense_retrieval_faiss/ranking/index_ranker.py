...
# 用于处理文档排名和检索任务。主要作用是根据输入查询和预先计算的文档向量对文档进行评分和排序
class IndexRanker():
    """
    初始化 IndexRanker 对象，接收文档向量（tensor）和文档长度（doclens）。
    计算文档长度的前缀和 doclens_pfxsum，并将其转换为张量。
    计算不同的步幅 strides，用于创建视图。
    调用 _create_views 和 _create_buffers 函数创建视图和缓冲区。
    """
    ...
    # 根据步幅创建不同视图，每个视图是以不同步幅滑动窗口方式划分的张量。
    ...
    # 对输入的查询和文档 ID 进行评分，返回文档的评分。
    # 计算每个文档的分数，使用不同的步幅视图进行计算。
