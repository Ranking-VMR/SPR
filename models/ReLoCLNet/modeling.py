...
from models.ReLoCLNet.contrastive import batch_video_query_loss
from models.ReLoCLNet.model_components import BertAttention, LinearLayer, BertSelfAttention, \
    TrainablePositionalEncoding, MILNCELoss, MILNCELoss_RVMR, MILNCELoss_Hard_Neg, MILNCELoss_RVMR_Hard_Neg2, \
    MILNCELoss_RVMR_Weighted
...
class ReLoCLNet(nn.Module):
    ...
    def forward(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask, st_ed_indices, match_labels, simi=None):
        """
        Args:
            query_feat: (N, Lq, Dq)
            query_mask: (N, Lq)
        ...
        return loss, {"loss_st_ed": float(loss_st_ed), "loss_fcl": float(loss_fcl), "loss_vcl": float(loss_vcl),
...
class ReLoCLNet_Hard_Neg(ReLoCLNet):
    def __init__(self, config):
        super(ReLoCLNet_Hard_Neg, self).__init__(config)
        self.nce_criterion = MILNCELoss_Hard_Neg(reduction='mean')
        logger.info(f"ReLoCLNet_Hard_Neg with MILNCELoss_Hard_Neg")
    
    @staticmethod
    def get_video_level_scores_hard_neg(modularied_query, hard_neg_context_feat, hard_neg_context_mask):
        """
        Args:
            modularied_query: (N, D)
            context_feat: (N, L, D), output of the first transformer encoder layer
            context_mask: (N, L)
        Returns:
            context_query_scores: (N)  score of each query w.r.t. corresponding video inside the batch.
        """
        modularied_query = F.normalize(modularied_query, dim=-1)
        hard_neg_context_feat = F.normalize(hard_neg_context_feat, dim=-1)
        
        query_context_scores = torch.einsum("nd,nld->nl", modularied_query, hard_neg_context_feat)  # (N, L)
        
        #context_mask = context_mask.transpose(0, 1).unsqueeze(0)  # (1, L, N)
        query_context_scores = mask_logits(query_context_scores, hard_neg_context_mask)  # (N, L)
        query_context_scores, _ = torch.max(query_context_scores, dim=1)  # (N)
        return query_context_scores
    
    @staticmethod
    def get_unnormalized_video_level_scores_hard_neg(modularied_query, hard_neg_context_feat, hard_neg_context_mask):
        """ Calculate video2query scores for each pair of video and query inside the batch.
        Args:
            modularied_query: (N, D)
            context_feat: (N, L, D), output of the first transformer encoder layer
            context_mask: (N, L)
        Returns:
            context_query_scores: (N, L)  score of each query w.r.t. corresponding video inside the batch.
        """
        query_context_scores = torch.einsum("nd,nld->nl", modularied_query, hard_neg_context_feat)  # (N, L)
    ...
    def get_pred_hard_neg(self, video_query, sub_query, hard_neg_video_feat, hard_neg_video_mask, hard_neg_sub_feat, hard_neg_sub_mask):
        
        # get video-level retrieval scores
        hard_neg_video_q2ctx_scores = self.get_video_level_scores_hard_neg(video_query, hard_neg_video_feat, hard_neg_video_mask)
        hard_neg_sub_q2ctx_scores = self.get_video_level_scores_hard_neg(sub_query, hard_neg_sub_feat, hard_neg_sub_mask)
        hard_neg_q2ctx_scores = (hard_neg_video_q2ctx_scores + hard_neg_sub_q2ctx_scores) / 2  # (N)
        
        return hard_neg_q2ctx_scores # (N)
    
    def get_video_level_loss_hard_neg(self, query_context_scores, hard_neg_query_context_scores):
        """ ranking loss between (pos. query + pos. video) and (pos. query + neg. video) or (neg. query + pos. video)
        Args:
            query_context_scores: (N, N), cosine similarity [-1, 1],
                Each row contains the scores between the query to each of the videos inside the batch.
            hard_neg_query_context_scores: (N), cosine similarity [-1, 1],
        """
        bsz = len(query_context_scores)
        diagonal_indices = torch.arange(bsz).to(query_context_scores.device)
        pos_scores = query_context_scores[diagonal_indices, diagonal_indices]  # (N, )

        loss_hard_neg_ctx = self.get_ranking_loss(pos_scores, hard_neg_query_context_scores)

        return loss_hard_neg_ctx
    
    def forward(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask, st_ed_indices, match_labels, hard_neg_video_feat, hard_neg_video_mask, hard_neg_sub_feat, hard_neg_sub_mask, simi=None):
        """
        Args:
            query_feat: (N, Lq, Dq)
            query_mask: (N, Lq)
            ...
            match_labels: (N, Lv), torch.LongTensor, matching labels for detecting foreground and background (not used)
        """
        # encode context and query features for positive samples
        video_feat, sub_feat, mid_x_video_feat, mid_x_sub_feat, x_video_feat, x_sub_feat = self.encode_context(
            video_feat, video_mask, sub_feat, sub_mask, return_mid_output=True)
        video_query, sub_query, query_context_scores, st_prob, ed_prob = self.get_pred_from_raw_query(
            query_feat, query_mask, x_video_feat, video_mask, x_sub_feat, sub_mask, cross=False,
        ...
        # encode context and query features for hard negative samples
        hard_neg_video_feat, hard_neg_sub_feat, hard_neg_mid_x_video_feat, hard_neg_mid_x_sub_feat, hard_neg_x_video_feat, hard_neg_x_sub_feat = self.encode_context(
            hard_neg_video_feat, hard_neg_video_mask, hard_neg_sub_feat, hard_neg_sub_mask, return_mid_output=True)
        
        #_, _, hard_neg_query_context_scores, _, _ = self.get_pred_from_raw_query(
        #    query_feat, query_mask, hard_neg_x_video_feat, hard_neg_video_mask, hard_neg_x_sub_feat, hard_neg_sub_mask, cross=False,
        #    return_query_feats=True)
        
        hard_neg_query_context_scores = self.get_pred_hard_neg(video_query, sub_query, hard_neg_x_video_feat, hard_neg_video_mask, hard_neg_x_sub_feat, hard_neg_sub_mask) # (N)
        ...
        if self.config.lw_vcl != 0:
            ...
            # hard negative
            hard_neg_mid_video_q2ctx_scores = self.get_unnormalized_video_level_scores_hard_neg(video_query, hard_neg_mid_x_video_feat, hard_neg_video_mask) # (N, L)
            hard_neg_mid_sub_q2ctx_scores = self.get_unnormalized_video_level_scores_hard_neg(sub_query, hard_neg_mid_x_sub_feat, hard_neg_sub_mask) # (N, L)
            hard_neg_mid_video_q2ctx_scores, _ = torch.max(hard_neg_mid_video_q2ctx_scores, dim=1) # (N)
            hard_neg_mid_sub_q2ctx_scores, _ = torch.max(hard_neg_mid_sub_q2ctx_scores, dim=1) # (N)
            hard_neg_mid_q2ctx_scores = (hard_neg_mid_video_q2ctx_scores + hard_neg_mid_sub_q2ctx_scores) / 2.0
            
            # exclude the contrastive loss for the same query
            loss_vcl = self.nce_criterion(mid_q2ctx_scores, hard_neg_q2ctx_scores=hard_neg_mid_q2ctx_scores)
            loss_vcl = self.config.lw_vcl * loss_vcl
        # moment localization loss
        loss_st_ed = 0 
        if self.config.lw_st_ed != 0:
            ...
            loss_st_ed = self.config.lw_st_ed * loss_st_ed
        # video level retrieval loss
        loss_neg_ctx, loss_neg_q, loss_hard_neg_ctx = 0, 0, 0
        if self.config.lw_neg_ctx != 0 or self.config.lw_neg_q != 0 or self.config.lw_hard_neg_ctx != 0:
            loss_neg_ctx, loss_neg_q = self.get_video_level_loss(query_context_scores)
            loss_neg_ctx = self.config.lw_neg_ctx * loss_neg_ctx
            loss_neg_q = self.config.lw_neg_q * loss_neg_q
            loss_hard_neg_ctx = self.get_video_level_loss_hard_neg(query_context_scores, hard_neg_query_context_scores)
            loss_hard_neg_ctx = self.config.lw_hard_neg_ctx * loss_hard_neg_ctx
        # sum loss
        loss = loss_fcl + loss_vcl + loss_st_ed + loss_neg_ctx + loss_neg_q + loss_hard_neg_ctx
        return loss, {"loss_st_ed": float(loss_st_ed), "loss_fcl": float(loss_fcl), "loss_vcl": float(loss_vcl),
                      "loss_neg_ctx": float(loss_neg_ctx), "loss_neg_q": float(loss_neg_q), "loss_hard_neg_ctx": float(loss_hard_neg_ctx), "loss_overall": float(loss)}

    

class ReLoCLNet_RVMR(ReLoCLNet):
    def __init__(self, config):
        super().__init__(config)
        self.nce_criterion = MILNCELoss_RVMR(reduction='mean')
        logger.info(f"ReLoCLNet_RVMR with MILNCELoss_RVMR")

    def forward(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask, st_ed_indices, match_labels, query_context_pos_mask, simi=None):
        """
        Args:
            query_feat: (N, Lq, Dq)
            query_mask: (N, Lq)
        ...
        if self.config.lw_vcl != 0:
            ...
            # exclude the contrastive loss for the same query
            mid_q2ctx_scores = (mid_video_q2ctx_scores + mid_sub_q2ctx_scores) / 2.0
            loss_vcl = self.nce_criterion(q2ctx_scores=mid_q2ctx_scores, q2ctx_pos_mask=query_context_pos_mask)
            loss_vcl = self.config.lw_vcl * loss_vcl
        # moment localization loss
        loss_st_ed = 0 
        if self.config.lw_st_ed != 0:
        ...
        loss_neg_ctx, loss_neg_q = 0, 0
        if self.config.lw_neg_ctx != 0 or self.config.lw_neg_q != 0:
            loss_neg_ctx, loss_neg_q = self.get_video_level_loss(query_context_scores, query_context_pos_mask)
            loss_neg_ctx = self.config.lw_neg_ctx * loss_neg_ctx
            loss_neg_q = self.config.lw_neg_q * loss_neg_q
        # sum loss
        loss = loss_fcl + loss_vcl + loss_st_ed + loss_neg_ctx + loss_neg_q
    ...
    def get_video_level_loss(self, query_context_scores, query_context_pos_mask):
        """ ranking loss between (pos. query + pos. video) and (pos. query + neg. video) or (neg. query + pos. video)
        Args:
            query_context_scores: (N, N), cosine similarity [-1, 1],
                Each row contains the scores between the query to each of the videos inside the batch.
            query_context_pos_mask: (N, N), 1 for positive pairs, 0 for negative pairs.
        """
        
        bsz = len(query_context_scores)
        pos_num_per_query = query_context_pos_mask.sum(dim=1)
        pos_num_per_context = query_context_pos_mask.sum(dim=0)
        
        # here we use the diagonal as positive pairs
        diagonal_indices = torch.arange(bsz).to(query_context_scores.device)
        pos_scores = query_context_scores[diagonal_indices, diagonal_indices]  # (N, )

        query_context_scores_masked = copy.deepcopy(query_context_scores.data)
        # impossibly large for cosine similarity, the copy is created as modifying the original will cause error
        query_context_scores_masked[query_context_pos_mask==1] = 999
        
        pos_query_neg_context_scores = self.get_neg_scores(query_context_scores, query_context_scores_masked, pos_num_per_query)
        neg_query_pos_context_scores = self.get_neg_scores(query_context_scores.transpose(0, 1),
                                                           query_context_scores_masked.transpose(0, 1),
                                                           pos_num_per_context)
        loss_neg_ctx = self.get_ranking_loss(pos_scores, pos_query_neg_context_scores)
    ...
    def get_neg_scores(self, scores, scores_masked, pos_num):
        """
        scores: (N, N), cosine similarity [-1, 1],
            Each row are scores: query --> all videos. Transposed version: video --> all queries.
        scores_masked: (N, N) the same as scores, except that the diagonal (positive) positions
            are masked with a large value.
        pos_num: (N, ), number of positive pairs for each row
        """
        ...
        sample_min_idx = pos_num.cpu().numpy().astype(int)  # skip the masked positive
        
        if self.config.use_hard_negative:
            sample_max_idx = sample_min_idx + self.config.hard_pool_size
            sample_max_idx = np.clip(sample_max_idx, a_min=None, a_max=bsz)
        else:
            sample_max_idx = np.ones_like(sample_min_idx) * bsz
        sample_max_idx = sample_max_idx.astype(int)
        
        random_neg_indices = torch.stack([torch.randint(sample_min_idx[i], sample_max_idx[i], (1,)) for i in range(bsz)], dim=0).squeeze(dim=-1).to(scores.device)
        sampled_neg_score_indices = sorted_scores_indices[batch_indices, random_neg_indices]
        sampled_neg_scores = scores[batch_indices, sampled_neg_score_indices]  # (N, )
        return sampled_neg_scores

class ReLoCLNet_RVMR_Weighted(ReLoCLNet):
    def __init__(self, config):
        super(ReLoCLNet_RVMR_Weighted, self).__init__(config)
        self.nce_criterion = MILNCELoss_RVMR_Weighted(reduction=False)
        self.temporal_criterion = nn.CrossEntropyLoss(reduction="none")
        logger.info(f"ReLoCLNet_RVMR_Weighted with MILNCELoss_RVMR_Weighted")
    ...
    def forward(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask, st_ed_indices, match_labels, query_context_pos_mask, simi=None):
        ...
        loss_fcl = 0
        if self.config.lw_fcl != 0:
            loss_fcl_vq = batch_video_query_loss(mid_x_video_feat, video_query, match_labels, video_mask, measure='JSD', reduction=False)
            loss_fcl_sq = batch_video_query_loss(mid_x_sub_feat, sub_query, match_labels, sub_mask, measure='JSD', reduction=False)
            loss_fcl = (loss_fcl_vq + loss_fcl_sq) / 2.0
            loss_fcl = self.config.lw_fcl * loss_fcl
        # video level contrastive learning loss (VideoCL)
        loss_vcl = 0
        ...
        loss_neg_ctx, loss_neg_q = 0, 0
        if self.config.lw_neg_ctx != 0 or self.config.lw_neg_q != 0:
            loss_neg_ctx, loss_neg_q = self.get_video_level_loss(query_context_scores, query_context_pos_mask, reduction=False)
            loss_neg_ctx = self.config.lw_neg_ctx * loss_neg_ctx
            loss_neg_q = self.config.lw_neg_q * loss_neg_q
        # sum loss
        loss_fcl = (loss_fcl * simi).mean() if self.config.loss_fcl_simi else loss_fcl.mean()
        loss_vcl = (loss_vcl * simi).mean() if self.config.loss_vcl_simi else loss_vcl.mean()
        loss_st_ed = (loss_st_ed * simi).mean() if self.config.loss_st_ed_simi else loss_st_ed.mean()
        loss_neg_ctx = (loss_neg_ctx * simi).mean() if self.config.loss_neg_ctx_simi else loss_neg_ctx.mean()
        loss_neg_q = (loss_neg_q * simi).mean() if self.config.loss_neg_q_simi else loss_neg_q.mean()
        loss = loss_fcl + loss_vcl + loss_st_ed + loss_neg_ctx + loss_neg_q
        return loss, {"loss_st_ed": float(loss_st_ed), "loss_fcl": float(loss_fcl), "loss_vcl": float(loss_vcl),
                      "loss_neg_ctx": float(loss_neg_ctx), "loss_neg_q": float(loss_neg_q), "loss_overall": float(loss)}
    
    def get_ranking_loss(self, pos_score, neg_score, reduction=True):
        """ Note here we encourage positive scores to be larger than negative scores.
        Args:
            pos_score: (N, ), torch.float32
            neg_score: (N, ), torch.float32
        """
        if self.config.ranking_loss_type == "hinge":  # max(0, m + S_neg - S_pos)
            ranking_loss = torch.clamp(self.config.margin + neg_score - pos_score, min=0)
        elif self.config.ranking_loss_type == "lse":  # log[1 + exp(S_neg - S_pos)]
            ranking_loss = torch.log1p(torch.exp(neg_score - pos_score))
        else:
            raise NotImplementedError("Only support 'hinge' and 'lse'")
        
        if reduction:
            return ranking_loss.sum() / len(pos_score)
        else:
            return ranking_loss

    
    def get_video_level_loss(self, query_context_scores, query_context_pos_mask, reduction=True):
        """ ranking loss between (pos. query + pos. video) and (pos. query + neg. video) or (neg. query + pos. video)
        Args:
            query_context_scores: (N, N), cosine similarity [-1, 1],
                Each row contains the scores between the query to each of the videos inside the batch.
        ...
        neg_query_pos_context_scores = self.get_neg_scores(query_context_scores.transpose(0, 1),
                                                           query_context_scores_masked.transpose(0, 1),
                                                           pos_num_per_context)
        loss_neg_ctx = self.get_ranking_loss(pos_scores, pos_query_neg_context_scores, reduction=reduction)
        loss_neg_q = self.get_ranking_loss(pos_scores, neg_query_pos_context_scores, reduction=reduction)
        return loss_neg_ctx, loss_neg_q
...
class ReLoCLNet_RVMR_Hard_Neg(ReLoCLNet_Hard_Neg):
    def __init__(self, config):
        super().__init__(config)
        self.nce_criterion = MILNCELoss_RVMR_Hard_Neg2(reduction='mean')
        logger.info(f"ReLoCLNet_RVMR_Hard_Neg with MILNCELoss_RVMR_Hard_Neg2")
    ...
    def forward(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask, st_ed_indices, match_labels, hard_neg_video_feat, hard_neg_video_mask, hard_neg_sub_feat, hard_neg_sub_mask, query_context_pos_mask, simi=None):
        """
        Args:
            query_feat: (N, Lq, Dq)
            query_mask: (N, Lq)
        ...
        if self.config.lw_vcl != 0:
            ...
            # exclude the contrastive loss for the same query
            loss_vcl = self.nce_criterion(mid_q2ctx_scores, q2ctx_pos_mask=query_context_pos_mask, hard_neg_q2ctx_scores=hard_neg_mid_q2ctx_scores)
            loss_vcl = self.config.lw_vcl * loss_vcl
        # moment localization loss
        loss_st_ed = 0 
        if self.config.lw_st_ed != 0:
